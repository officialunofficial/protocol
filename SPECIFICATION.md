...
Quota eligibility and storage-grant expiry sweeps MUST be evaluated at block/proposal time, not at `MessageData.timestamp`. Submit and dry-run paths that do not yet know the final block timestamp MAY use current node time as a best-effort admission check, but block execution is authoritative. `STORAGE_CLAIM` grant validity is different: the new grant's `expires_at = settlement_block_timestamp + STORAGE_TOTAL_PERIOD` MUST still be compared against `MessageData.timestamp` to reject claims that were already expired at the claimed message time.

Message timestamps remain semantically meaningful for ordering. LWW registers and tombstone-backed 2P sets MUST use `MessageData.timestamp` for conflict resolution; block/proposal time is not a replacement for author-declared ordering time.

Authorization and existence checks are evaluated against the current canonical state at execution time. Backfillable timestamps do not trigger historical state reconstruction: a backdated message must still be valid against the state as of the containing block.
...
2P add/remove ordering uses `MessageData.timestamp`. Block/proposal time is used only by quota eligibility checks that may precede these conflict-resolution rules.
...
- **`STORAGE_CLAIM`:** Finalized settlement evidence must match `owner_address`, `actor`, and `units`; expiry derives from the finalized settlement block timestamp. If the claim marker already exists, the message is a valid duplicate claim and execution is an idempotent no-op. Otherwise claimant expired storage grants MUST be swept at block/proposal time, the raw storage grant MUST be added, and cached `AccountState.storage_units` MUST be refreshed. A newly created grant whose expiry is already at or before `MessageData.timestamp` MUST be rejected. `STORAGE_CLAIM` does not assign or update usernames.
- **`USERNAME_CREATE`:** Delegated-key authorization with required scope `SIGNING` must pass. Claimant expired storage grants MUST be swept at block/proposal time. The claimant must have active storage after sweep and must not already have an active username. The requested username must be available after mandatory stale-reservation reclamation, also evaluated at block/proposal time. On success, execution MUST set `AccountState.username_last_set_at = MessageData.timestamp`.
- **`USERNAME_UPDATE`:** Delegated-key authorization with required scope `SIGNING` must pass. Claimant expired storage grants MUST be swept at block/proposal time. The claimant must have active storage after sweep and must already have an active username. The current username index entry MUST authenticate `[0x08 | current_username] -> owner_address`, otherwise execution MUST fail closed. The requested username must differ from the current username and must be available after mandatory stale-reservation reclamation, also evaluated at block/proposal time. The message timestamp MUST be at least `AccountState.username_last_set_at + USERNAME_CHANGE_COOLDOWN`, using saturating `uint32` arithmetic for the comparison. On success, execution MUST set `AccountState.username_last_set_at = MessageData.timestamp`.
...
|---------|------|---------|
| 2026.5.4 | 2026-05-14 | Clarify backfillable timestamp semantics: only storage identity messages require fresh timestamps, quota sweeps use block/proposal time, conflict resolution uses message timestamps, and authorization is evaluated against current state. |
